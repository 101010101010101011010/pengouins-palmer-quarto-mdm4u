...
dfTab <- penguins %>% mutate(flipper_length_mm = flipper_length_mm / 10) %>% filter(!is.na(flipper_length_mm) | !is.na(sex)) %>% group_by(species) %>% select(species, bill_length_mm, bill_depth_mm, flipper_length_mm) %>%
  summarize(
...
ggplot(penguins %>% mutate(flipper_length_mm = flipper_length_mm / 10) %>% filter(!is.na(flipper_length_mm) | !is.na(sex)), aes(bill_length_mm, bill_depth_mm)) +
